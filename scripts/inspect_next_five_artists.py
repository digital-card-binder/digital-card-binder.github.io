...
def identities(row):
    ...
    setv=str(row.get("set") or "").strip()
    numv=str(row.get("cardNumber") or row.get("number") or "").strip()
...
with tempfile.TemporaryDirectory() as td:
    ...
    for artist in ARTISTS:
        allrows=grouped[artist]; relevant=[]; matched=[]; unresolved=[]
        for era,setcode,num,rel,text in allrows:
            vals=idx.get((setcode.casefold(),num),[])
            proven=bool(vals) or (era,setcode) in krsets
            if not proven: continue
            relevant.append(rel)
            valid=[r for r in vals if row_name(r)]
            if valid: matched.append((rel,valid))
            else: unresolved.append((rel,names(text),field(text,'rarity'),field(text,'category')))
        print("\nARTIST",artist,"VARIANTS",sorted(variants[artist]))
        print("ALL",len(allrows),"KOREA_RELEVANT",len(relevant),"MATCHED",len(matched),"UNRESOLVED",len(unresolved))
        for rel,nm,rar,cat in unresolved:
            print("UNRESOLVED",json.dumps({"path":rel,"names":nm,"rarity":rar,"category":cat},ensure_ascii=False))
