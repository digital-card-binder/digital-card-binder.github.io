...
CANDIDATES=["Naoki Saito","kawayoo","Oswaldo KATO","kantaro","Saboteri"]
records=[]
def walk(node,src):
...
for r in records:
    ...
    if im:
        directory,filename=im.groups(); nm=re.search(r'_0*(\d{1,4})(?:[_\.]|$)',filename,re.I)
    ...
    for code,num in pairs:
        key=(code.lower(),str(int(num))); ident=(r.get('name',''),r.get('image',''),r.get('source',''))
        if not any((x.get('name',''),x.get('image',''),x.get('source',''))==ident for x in idx[key]): idx[key].append(r)

def field(txt,key):
    m=re.search(r'\b'+re.escape(key)+r':\s*["\']([^"\']*)["\']',txt)
    return m.group(1) if m else ''
def names(txt):
    m=re.search(r'name:\s*\{(.*?)\}\s*,',txt,re.S)
    if not m: return {}
    ...
    for root,_,files in os.walk(base):
        for fn in files:
            ...
            for artist in CANDIDATES:
                if re.search(r'illustrator:\s*["\']'+re.escape(artist)+r'["\']',txt): hits[artist].append((os.path.relpath(path,base),txt))
    for artist in CANDIDATES:
        unresolved=[]; matched=0
        for p,txt in hits[artist]:
            parts=p.split(os.sep)
            if len(parts)<3: continue
            era,setcode=parts[0],parts[1]; stem=os.path.splitext(parts[-1])[0]; m=re.match(r'^0*(\d+)$',stem)
            ...
            if vals: matched+=1
                ...
                unresolved.append({'path':p,'names':names(txt),'rarity':field(txt,'rarity'),'category':field(txt,'category')})
        print('\nARTIST',artist,'MATCHED',matched,'UNRESOLVED',len(unresolved))
        for item in unresolved: print(json.dumps(item,ensure_ascii=False))
