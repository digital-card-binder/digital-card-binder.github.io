...
$TempScript = Join-Path $env:TEMP 'rebuild-official-artist-dex-v4.ps1'

Write-Host '[runner-v4] Fetching rebuild script from GitHub API...'
...
Write-Host '[runner-v4] Applying the official first-page/load-more AJAX behavior...'
...
function Search-Artist([string]$Artist) {
  ...
  while ($true) {
    ...
    # The live page behaves differently for the initial search and load-more.
    # Initial AJAX: search_params=all and no explicit limit.
    # Load-more AJAX: limit=<returned limit> and no search_params.
    if ($limit -eq 0) {
      $curlArgs += @('-F','search_params=all')
    } else {
      $curlArgs += @('-F',("limit=$limit"))
    }
    $curlArgs += @("$Base/v2/ajax2_dev2",'-o',$respPath)
    ...
    # PowerShell 5.1 can expose numeric JSON object members unusually. If that
    # happens, reparse the result object through JavaScriptSerializer.
...
Write-Host '[runner-v4] Patching large-file GitHub uploads...'
...
'@
$patched = [regex]::Replace($patched, $uploadPattern, $uploadReplacement)
if ($patched -eq $text) { throw 'Could not patch Upload-File.' }

Write-Host '[runner-v4] Patching Windows PowerShell collection edge cases...'
...
Write-Host '[runner-v4] UTF-8 BOM verified.'
Write-Host '[runner-v4] Starting official rebuild. This can take several minutes.' -ForegroundColor Cyan
