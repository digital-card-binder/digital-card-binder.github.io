...
## Series master inventory

- `data/series-inventory.json` is the deterministic baseline of the effective series catalog after merging `data/series.json` and `data/series-legacy.json` by set code.
- The inventory is generated by `scripts/build-series-inventory.mjs` and currently records 200 sets / 15,670 cards across ORIGIN, ADV, DP, BW, XY, SM, S, SV, and M.
- It records era/set counts, completeness counters, source/image hosts, per-set first/last card markers, duplicate identity counts, and SHA-256 digests. It does not duplicate the full card payload or alter ownership data.
- Some current-series groups omit the explicit `era` field; the inventory infers S/SV/M from set code for reporting only and does not rewrite canonical catalogs.
- Do not hand-edit the generated inventory. Use `npm run series-inventory:sync`; `npm run series-inventory:check` runs before the rest of the site checks.
- This baseline is the comparison anchor for the later Pokemon Korea official-catalog reconciliation. A mismatch must be reviewed before changing canonical cards or user ownership keys.
