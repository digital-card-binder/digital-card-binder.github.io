...
- `나만 보기`, `공개` 두 단계의 도감별 공개
- 공개 프로필과 기존 도감 UI를 재사용한 읽기 전용 상세 화면
- 공개 도감이 하나 이상인 프로필만 자동 등재하는 공개 컬렉터 보드
- 링크 공개 토큰 생성·도감 링크 복사 UI는 제공하지 않음
...
보내지는 않습니다.
전체 Sheets 동기화는 프로모 마스터 목록 변경으로 생긴 과거 공개 데이터 차이를
정리할 수 있도록, 시트 수정 행이 없어도 팩 projection을 한 번 재생성합니다.
...
| `users/{uid}/collectionSettings/{collectionId}` | `dashboardVisible`, `visibility`, `displayOrder`, 호환용 `shareId` | 본인만 읽기·쓰기 |
...
| `collectorShareOwners/{shareId}` | 과거 링크 공개 호환 데이터이며 신규 생성하지 않음 | 해당 소유자만 exact get |
...
| `publicCollectorDirectory/{publicId}` | 무작위 `publicId`, 갱신 시각 | 공개 get/list, 공개 도감 소유자만 쓰기 |
| `sharedCollections/{shareId}` | 과거 링크 공개 호환 데이터이며 신규 화면에서 읽지 않음 | 기존 Rules 보존 |
...
공개 보드는 `publicProfiles` 전체 list를 열지 않습니다. 공개 도감이 하나 이상일
때만 별도의 디렉터리에 무작위 `publicId`를 등재하고, 화면이 해당 ID의 공개 프로필과
공개 projection을 각각 읽습니다. 나만 보기 도감만 남으면 디렉터리 항목을
삭제하며, 오래된 항목이 남더라도 공개 projection이 0개이면 화면에서 제외합니다.
...
새 사이트는 `#share` 토큰을 읽지 않고 링크 공개 토큰을 새로 만들지 않습니다.
과거 링크 공개 데이터는 안전 원칙에 따라 이 변경에서 자동 삭제하지 않습니다.
...
변경 자체가 거부됩니다. 공개 projection의 직접 삭제도 해당 설정이 더는
공개 범위를 요구하지 않을 때만 허용합니다.
...
- 공개 팩도감은 공식 프로모팩과 행사·배포 프로모 카드 ID를 모두 projection에 포함하고,
  직접 등록한 커스텀 프로모의 상세 정보는 제외
