...
실행 `35700074440`의 29개 배치 보고서 전체를 합치면 현대 이미지 48개, 구형 이미지 113개, 총 161개 경로가 실패했다. 실행 `35802010751`에서 159개(현대 47개·구형 112개)를 채워 캐시에 저장하고 총 16,660개의 WebP 파일을 검사했다. 현대 검증이 먼저 중단되어 구형 누락은 첫 오류에 표시되지 않았다.
...
메가브레이브 한글판 수록 목록은 092/063까지이므로 중복 등록된 093번 항목을 삭제하고 `92/063`으로 정정했다. 093번 뒤에 있는 세 장의 보유 기록 키는 `accountIndex`로 기존 값을 보존했다. 이미지 목록은 16,661개가 되었으며 구형 풀 에너지 한 장은 검증 대상에 그대로 남는다. 해당 카드의 정확한 한글판 사진을 구할 때까지 전체 배포를 중단한다. 누락을 허용하거나 임시 이미지를 넣어 성공으로 처리하지 않는다.
